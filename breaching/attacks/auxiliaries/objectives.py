...
class GradientLoss(torch.nn.Module):
    ...
    def _grad_fn_single_step(self, model, candidate, labels):
        ...
        model.zero_grad()
        predict = model(candidate)
        task_loss = self.loss_fn(predict, labels)
...
class LayerPickedEuclidean(GradientLoss):
    """Gradient matching based on the euclidean distance of two gradient vectors."""

    def __init__(self, scale=1.0, start=0, broken_tail=0, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.task_regularization = task_regularization
        self.start = start
    ...
    def __repr__(self):
        return f"Layer Picked Euclidean loss with scale={self.scale} and task reg={self.task_regularization}"

    def _euclidean(self, gradient_rec, gradient_data):
        len_layer = len(gradient_rec)
...
class DynaLayerPickedEuclidean(GradientLoss):
    ...
    def __init__(self, scale=1.0, start=0, min_start=0, broken_tail=0, peroid_Add10=1000, add10=10, min_weight_scale=1, max_weight_scale=1, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
        self.min_start = min_start
        self.broken_tail = broken_tail
        self.peroid_Add10 = peroid_Add10
        self.add10 = add10
    ...
    def __repr__(self):
        return f"Layer Weighted Picked Cosine Similarity with scale={self.scale} and task reg={self.task_regularization}. Start layer={self.start} and broken tail={self.broken_tail}"

    def gradient_based_loss(self, gradient_rec, gradient_data):
        len_layer = len(gradient_rec)
        final = len_layer - self.broken_tail
        objective = 0.0
        if (self.iter+1) % self.peroid_Add10 == 0:
...
class DynaLayerPickedEuclideanKeep10(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """
...
class DynaLayerPickedEuclideanDelete0(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            if self.start <= count+1 <= final:
                mask = torch.zeros_like(data) != data
                objective += (rec*mask - data*mask).pow(2).sum()
...
class LayerPickedCosineSimilarity(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """
    ...
    def __repr__(self):
        return f"Layer Picked Cosine Similarity with scale={self.scale} and task reg={self.task_regularization}. Start layer={self.start} and broken tail={self.broken_tail}"
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        scalar_product, rec_norm, data_norm = 0.0, 0.0, 0.0
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            if count+1 >= self.start and count+1 <= final:
                scalar_product += (rec * data).sum()
                rec_norm += rec.pow(2).sum()
                data_norm += data.pow(2).sum()
...
class LayerValuePickedCosineSimilarity(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """

    def __init__(self, scale=1.0, mask_value=1e-6, start=0, broken_tail=0, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
        self.broken_tail = broken_tail
    ...
    def __repr__(self):
        return f"Layer&Value Picked Cosine Similarity with scale={self.scale} and task reg={self.task_regularization}. Start layer={self.start} and broken tail={self.broken_tail}"
...
class LayerWeightedPickedCosineSimilarity(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """

    def __init__(self, scale=1.0, start=0, broken_tail=0, min_weight_scale=0, max_weight_scale=1, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
        self.broken_tail = broken_tail
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            if self.start <= count+1 <= final:
                weight = (count+1-self.start)/(final-self.start)*(self.max_weight_scale-self.min_weight_scale)+self.min_weight_scale
                scalar_product += (rec * data).sum()*weight
                rec_norm += rec.pow(2).sum()*weight
                data_norm += data.pow(2).sum()*weight
...
class DynaLayerPickedCosineSimilarity(GradientLoss):
    """Gradient matching based on cosine similarity of two gradient vectors.
    All positions that are zero in the data gradient are masked.
    """

    def __init__(self, scale=1.0, start=0, min_start=0, broken_tail=0, peroid_Add10=1000, add10=10, min_weight_scale=0, max_weight_scale=1, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
        self.min_start = min_start
...
class DynaLayerRandPickedCosineSimilarity(GradientLoss):
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        if (self.iter+1) % self.peroid_Add10 == 0:
            self.start = self.start-self.add10 if (self.start-self.add10) > self.min_start else self.min_start
        self.iter += 1
        if self.iter >= self.peroid_Add10+1:
            # flag = True
            self.min_weight_scale = 1
            self.max_weight_scale = 1
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            if self.start <= count+1 <= final:
                mask = torch.rand_like(data) > 0.3
                weight = (count+1-self.start)/(final-self.start)*(self.max_weight_scale-self.min_weight_scale)+self.min_weight_scale
                # if count+1 < 153 and self.iter < 200:
                #     weight = weight*0.1
...
class DynaLayerRandPickedZhucenCosineSimilarity(GradientLoss):
    def __init__(self, scale=1.0, start=0, min_start=0, broken_tail=0, peroid_Add10=1000, add10=10, min_weight_scale=0, max_weight_scale=1, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            scalar_product, rec_norm, data_norm = 0.0, 0.0, 0.0
            if self.start <= count+1 <= final:
                mask = torch.rand_like(data) > 0.0  # 0.2
                weight = (count+1-self.start)/(final-self.start)*0.5+0.5
                scalar_product += (rec * data * mask).sum()
                rec_norm += (rec * mask).pow(2).sum()
                data_norm += (data * mask).pow(2).sum()
                if data_norm < 1e-10 or rec_norm < 1e-10:
                    continue
                objective += (1 - scalar_product / rec_norm.sqrt() / data_norm.sqrt())*weight
                total_used_layer += 1

        objective = objective/total_used_layer

        return objective * self.scale



class DynaLayerRandSuperPickedCosineSimilarity(GradientLoss):
    """abandon some layers in the middle"""

    def __init__(self, scale=1.0, start=0, min_start=0, broken_tail=0, min_ban=-1, max_ban=1000, peroid_Add10=1000, add10=10, min_weight_scale=0, max_weight_scale=1, task_regularization=0.0, **kwargs):
        super().__init__()
        self.scale = scale
        self.start = start
        self.min_start = min_start
    ...
    def gradient_based_loss(self, gradient_rec, gradient_data):
        ...
        for count, (rec, data) in enumerate(zip(gradient_rec, gradient_data)):
            if self.start <= count+1 <= final:
                if count+1 > self.max_ban or count+1 < self.min_ban:
                    mask = torch.rand_like(data) > 0.1  # 0.2
                    weight = 1
                    # if count+1 < 153 and self.iter < 200:
                    #     weight = weight*0.1
                    scalar_product += (rec * data * mask).sum()*weight
...
objective_lookup = {
    ...
    "layer-pick-cosine-similarity": LayerPickedCosineSimilarity,
    "layer-pick-euclidean": LayerPickedEuclidean,
    "layer-value-pick-cosine-similarity": LayerValuePickedCosineSimilarity,
    "layer-weighted-pick-cosine-similarity": LayerWeightedPickedCosineSimilarity,
    "dyna-layer-pick-cosine-similarity": DynaLayerPickedCosineSimilarity,
    "dyna-layer-pick-euclidean": DynaLayerPickedEuclidean,
    "dyna-layer-rand-cosine-similarity":DynaLayerRandPickedCosineSimilarity,
    "dyna-layer-rand-super-cosine-similarity":DynaLayerRandSuperPickedCosineSimilarity,
    "dyna-layer-rand-cosine-similarity-zhuceng":DynaLayerRandPickedZhucenCosineSimilarity,
    "dyna-layer-pick-euclidean-fckeep10": DynaLayerPickedEuclideanKeep10,
    "dyna-layer-pick-euclidean-delete0": DynaLayerPickedEuclideanDelete0,
}
